...
while(<IN>){
	...
	while(<FILE>){
		...
		@tmp=split/\t/,$_;
		if(!/^\#/){
			for(my $ix=4;$ix<@tmp;$ix++){
				push(@{$hash{$line[2]}{$line[1]}{$tmp[3]}},$tmp[$ix]);
			}
		}
	}
	close FILE;
...
my $h=$w*0.618;#height of the graph
...
=cut
		for my $x(sort{$a<=>$b} keys $hash{$sample}{$feature}){
			for(sort @{$hash{$sample}{$feature}{$x}}){print "$_\n";
				if($_ && abs($_)<=$maxy[$j]){
					#collect all the dots that are in the range
					push(@{$pos{$x}},$_);
				}
		...
		my $win=($w-2*$m)/@maxy;
		for my $x(keys %pos){
			my $x1=$center_x+$x*((($w-2*$m)/($wn))/(2*$dis));
			for(sort @{$pos{$x}}){
				my $y2=$center_y+$_*((($h-2*$m)/($hn*2))/($maxy[$j]));
				$im->filledEllipse($x1, $y2, 0.01*$win, 0.0001*$win,$purple);
			}
		}
